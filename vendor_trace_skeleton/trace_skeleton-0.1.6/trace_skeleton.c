...
// PARAMS
//================================
#define SKEL__SAVE_RECTS \
    0  // additionally save bounding rects of chunks (for visualization)
 ...
 *             MSB = is matching the left (not right) end of the fragment from
 * first  chunk LSB = is matching the right (not left) end of the fragment from
 * second chunk
    ...
    if (fsize == 2) {  // probably just a line, connect them
        ...
        for (int i = y + 1; i < y + h - 1; i++) {
            for (int j = x + 1; j < x + w - 1; j++) {
                int s = (im[i * W - W + j - 1]) + (im[i * W - W + j]) +
                        (im[i * W - W + j - 1 + 1]) + (im[i * W + j - 1]) +
                        (im[i * W + j]) + (im[i * W + j + 1]) +
                        (im[i * W + W + j - 1]) + (im[i * W + W + j]) +
                        (im[i * W + W + j + 1]);
                if (s > ms) {
                    mi = i;
                    mj = j;
 ...
 * Algorithm:
 * 1. if chunk size is small enough, reach recursive bottom and turn it into
 * segments
 * 2. attempt to split the chunk into 2 smaller chunks, either horizontall or
 * vertically; find the best "seam" to carve along, and avoid possible
 * degenerate cases
 ...
 */
polyline_t* trace_skeleton(int x, int y, int w, int h, int iter, int csize,
                           int maxIter) {
    polyline_t* frags = NULL;
    ...
    if (h > csize) {  // try splitting top and bottom
        for (int i = y + 3; i < y + h - 3; i++) {
            if (im[i * W + x] || im[(i - 1) * W + x] || im[i * W + x + w - 1] ||
                im[(i - 1) * W + x + w - 1]) {
                continue;
            }
            int s = 0;
            ...
            if (s < ms) {
                ...
                mi = i;
                       ...
                       abs(i - (y + h / 2)) < abs(mi - (y + h / 2))) {
                // if there is a draw (very common), we want the seam to be near
                // the middle to balance the divide and conquer tree
                ms = s;
                mi = i;
            }
        }
    ...
    if (w > csize) {  // same as above, try splitting left and right
        for (int j = x + 3; j < x + w - 3; j++) {
            if (im[W * y + j] || im[W * (y + h) - W + j] || im[W * y + j - 1] ||
                im[W * (y + h) - W + j - 1]) {
                continue;
            }
            int s = 0;
    ...
    if (dr != 0 &&
        not_empty(L0, L1, L2,
                  L3)) {  // if there are no white pixels, don't waste time
#if SKEL__SAVE_RECTS
...
#endif
        frags = merge_frags(
            frags, trace_skeleton(R0, R1, R2, R3, iter + 1, csize, maxIter), sx,
            dr);
    }

    if (mi == -1 &&
        mj == -1) {  // splitting failed! do the recursive bottom instead
        frags = chunk_to_frags(x, y, w, h);
    }
    return frags;
}
